...
func operatorCaller() domain.Caller {
	return domain.Caller{Principal: domain.PrincipalOperator}
}
...
func (f *fakeSource) Status(_ context.Context) (frameworkmcp.Status, error) {
...
func (f *fakeSource) SubmitOrderToken(
	...
	return frameworkmcp.SubmitOrderTokenResult{}, nil
}
...
// callHealth calls the health tool.
func callHealth(t *testing.T, src frameworkmcp.Source) *toolResult[healthOutput] {
	t.Helper()
	return callTool[healthOutput](t, src, healthToolName, healthInput{})
}

// callGetAccountState calls the get_account_state tool.
func callGetAccountState(
	t *testing.T, src frameworkmcp.Source, account string,
) *toolResult[getAccountStateOutput] {
	t.Helper()
	return callTool[getAccountStateOutput](
		t, src, getAccountStateToolName, getAccountStateInput{Account: account},
	)
}

// callGetLimits calls the get_limits tool.
func callGetLimits(
	t *testing.T, src frameworkmcp.Source, account string,
) *toolResult[getLimitsOutput] {
	t.Helper()
	return callTool[getLimitsOutput](t, src, getLimitsToolName, getLimitsInput{Account: account})
}

// callGetAudit calls the get_audit tool.
func callGetAudit(
	t *testing.T, src frameworkmcp.Source, limit int,
) *toolResult[getAuditOutput] {
	t.Helper()
	return callTool[getAuditOutput](t, src, getAuditToolName, getAuditInput{Limit: limit})
}

// callGetOrder calls the get_order tool.
func callGetOrder(
	t *testing.T, src frameworkmcp.Source, orderExternalID string,
) *toolResult[getOrderOutput] {
	t.Helper()
	return callTool[getOrderOutput](
		t, src, getOrderToolName, getOrderInput{OrderExternalID: orderExternalID},
	)
}

// callCheckOrder calls the check_order tool.
func callCheckOrder(
	t *testing.T, src frameworkmcp.Source, in checkOrderInput,
) *toolResult[checkOrderOutput] {
	t.Helper()
	return callTool[checkOrderOutput](t, src, checkOrderToolName, in)
}
...
// assertNoSurrogateID fails if the structured tool payload, as the client
// received it, carries any forbidden id field. It returns true on a clean
// payload so callers can assert inline.
func assertNoSurrogateID(t *testing.T, wire json.RawMessage) bool {
	t.Helper()
	clean := true
	for _, field := range noSurrogateIDFields {
		if strings.Contains(string(wire), field) {
			t.Errorf("forbidden id field %s present in MCP output: %s", field, wire)
			clean = false
		}
	}
	return clean
...
func TestGetAccountStateHappyPath(t *testing.T) {
	...
	if !assertNoSurrogateID(t, res.Wire) {
...
func (c *captureNSource) Status(_ context.Context) (frameworkmcp.Status, error) {
	return frameworkmcp.Status{}, nil
}
...
func TestGetOrderHappyPath(t *testing.T) {
	...
	assertNoSurrogateID(t, res.Wire)
}
...
func TestSetMarketDataInstrumentGatedAndDelegates(t *testing.T) {
	...
	res := callSetMarketDataInstrument(t, disabled, setMarketDataInstrumentInput{
	...
	enabled := &fakeSource{}
	res = callSetMarketDataInstrument(t, enabled, setMarketDataInstrumentInput{
		InstanceExternalID: "mock-1", ExternalSymbol: "AAPL", Enabled: true,
	})
	requireNotToolError(t, res.IsError)
