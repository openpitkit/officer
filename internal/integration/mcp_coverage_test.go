...
// callSetMarketDataInstrument calls the set_market_data_instrument tool with the
// given input and fails on a protocol-level error.
func callSetMarketDataInstrument(
	t *testing.T, src frameworkmcp.Source, in setMarketDataInstrumentInput,
) *toolResult[setMarketDataInstrumentOutput] {
	t.Helper()
	return callTool[setMarketDataInstrumentOutput](
		t, src, setMarketDataInstrumentToolName, in,
	)
}
...
func (s *setMDErrSource) Status(context.Context) (frameworkmcp.Status, error) {
...
func TestNewServerNilSource(t *testing.T) {
	...
	srv, err := frameworkmcp.Build(
		officerToolRegistry(), nil, nil, httpx.AllowAll{}, operatorCaller(),
	)
	if err == nil {
...
func TestHandlerReturnsHTTPHandler(t *testing.T) {
	t.Parallel()
	h, err := frameworkmcp.Handler(
		officerToolRegistry(), &fakeSource{}, nil, httpx.AllowAll{},
		func(*http.Request) (domain.Caller, error) { return operatorCaller(), nil },
	)
	if err != nil {
...
func TestHandlerNilSource(t *testing.T) {
	t.Parallel()
	h, err := frameworkmcp.Handler(
		officerToolRegistry(), nil, nil, httpx.AllowAll{},
		func(*http.Request) (domain.Caller, error) { return operatorCaller(), nil },
	)
	if err == nil {
