...
var clientDataAllowlist = []clientDataAllow{
	...
	},
	{
		...
		Kind:    "business-csv-uncovered",
		Why:     "orders are export-only business CSV rows and omit internal reservation state",
	},
	{
		Surface: driftSchema,
		Entity:  "order_record",
...
func newClientDataDriftRealm(
	...
	t.Helper()
	st, err := sqlite.New(t.TempDir()+"/client-data-drift.db", domain.DefaultRealm)
	if err != nil {
...
func seedClientDataDriftRealm(
	...
	must(t, "AppendAudit", rs.AppendAudit(ctx, store.AuditEntry{
		...
		Verdict:      "reject",
		RejectCode:   "order_qty_exceeds_limit",
		Actor:        "operator",
