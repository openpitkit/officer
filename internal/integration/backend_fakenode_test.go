...
type fakeNode struct {
	...
	nextOrderSeq            byte
	nextEventSeq            int
	submitResult            *fwengine.OrderResult
	immediateResult         *fwengine.ImmediateResult
	...
	// Canned list pages and the last filter each list method saw.
...
func (n *fakeNode) Realm() domain.RealmID { return domain.DefaultRealm }
...
func (n *fakeNode) SetAccountBlocked(
	_ context.Context, account domain.AccountID, blocked bool, reason string,
	missing domain.MissingAccountPolicy, _ domain.Caller,
) error {
	n.missingAccountCalls = append(n.missingAccountCalls, missing)
	n.blockCalls = append(n.blockCalls, blockCall{account, blocked, reason})
	return nil
}
...
func (n *fakeNode) GetAccountState(
	...
	return domain.Account{Code: code}, n.limits, nil
}
...
func (n *fakeNode) SetAccountCurrency(
	context.Context, domain.AccountID, string, domain.Caller,
) error {
	return nil
}
...
func (n *fakeNode) DeleteAccount(
	context.Context, domain.AccountID, bool, domain.Caller,
) error {
	return nil
}
...
func (n *fakeNode) ApplyAdjustment(
	_ context.Context, account domain.AccountID, externalID domain.ExternalID,
...
func (n *fakeNode) SubmitOrderWithAttestation(
	...
	caller domain.Caller,
	attestFor func(domain.Order, fwengine.OrderResult) store.EventAttestor,
) (domain.Order, fwengine.OrderResult, error) {
	snapshot := n.snapshotOrderTxn()
	before := len(n.orderEvents[o.ExternalID])
	order, err := n.SubmitOrder(ctx, o, missing, caller)
	if err != nil {
		return domain.Order{}, fwengine.OrderResult{}, err
	}
	if o.ExternalID.IsZero() {
		before = 0
	}
	result := fwengine.OrderResult{
		Accepted:            true,
...
func (n *fakeNode) SubmitImmediate(
	_ context.Context, o domain.Order,
	missing domain.MissingAccountPolicy, _ domain.Caller,
) (domain.Order, fwengine.ImmediateResult, error) {
	n.missingAccountCalls = append(n.missingAccountCalls, missing)
	if n.submitErr != nil {
		return domain.Order{}, fwengine.ImmediateResult{}, n.submitErr
	}
	order := o
	order.ExternalID = n.recordedOrderExternalID(o)
	n.appendEvent(order.ExternalID, domain.OrderEventSubmitted, domain.OrderEventPayload{})
	...
	return order, completeFakeImmediateResult(order, fwengine.ImmediateResult{
		Accepted:            true,
		SettlementLockPrice: "100",
...
func completeFakeImmediateResult(
	order domain.Order, result fwengine.ImmediateResult,
	...
	if result.Persistence == nil {
		result.Persistence = &fwengine.ExecutionReportPersistence{
			Trade: &domain.Trade{
				Order:      order.ExternalID,
				Account:    order.Account,
				BaseAsset:  order.BaseAsset,
...
func (n *fakeNode) SubmitImmediateWithAttestation(
	...
	missing domain.MissingAccountPolicy,
	caller domain.Caller,
	attestFor func(domain.Order, fwengine.ImmediateResult) store.EventAttestor,
) (domain.Order, fwengine.ImmediateResult, error) {
	snapshot := n.snapshotOrderTxn()
	before := len(n.orderEvents[o.ExternalID])
	order, result, err := n.SubmitImmediate(ctx, o, missing, caller)
	if err != nil {
		return domain.Order{}, fwengine.ImmediateResult{}, err
	}
	if result.Accepted {
		n.appendEvent(order.ExternalID, domain.OrderEventCommitted, domain.OrderEventPayload{})
...
func (n *fakeNode) CancelOrder(
	...
	_ domain.Caller,
) (domain.Order, fwengine.ExecutionReportResult, error) {
	order := n.orders[orderID]
	if n.cancelNoop {
		return order, fwengine.ExecutionReportResult{}, nil
	}
	if n.cancelErr != nil {
		return domain.Order{}, fwengine.ExecutionReportResult{}, n.cancelErr
	}
	for _, event := range n.orderEvents[orderID] {
		if event.Payload.ExecutionReport != nil {
			return domain.Order{}, fwengine.ExecutionReportResult{},
	...
	if _, err := domain.ExecutionReportRequiresEngine(in); err != nil {
		return domain.Order{}, fwengine.ExecutionReportResult{}, err
	}
	...
	persistence := fwengine.ExecutionReportPersistence{
		OrderStatus: domain.OrderStatusCancelled,
		Leaves:      leavesQuantity,
	}
	return order, fwengine.ExecutionReportResult{Persistence: &persistence}, nil
}
...
func (n *fakeNode) ApplyExecutionReport(
	_ context.Context, in domain.ExecutionReportInput, caller domain.Caller,
...
func newTestServiceWithMarketDataRuntime(
	...
	t.Helper()
	registry := firstPartyMarketDataRegistry(t)
	...
	fn := &fakeNode{orders: make(map[domain.ExternalID]domain.Order)}
	svc := newOfficerService(t, fn, md, nil)
	return svc, fn
}
...
func newTestServiceWithSigner(
	...
	t.Helper()
	fn := &fakeNode{orders: make(map[domain.ExternalID]domain.Order)}
	svc := newOfficerService(t, fn, nil, signer)
	return svc, fn
}

// newOfficerService builds the service the Officer composition builds over n:
// the native engine's lock decoder, the first-party market-data providers, and
// the Officer MCP catalogue.
func newOfficerService(
	t *testing.T, n node.Node, md backend.MarketDataRuntime, signer fwsigning.Service,
) *backend.Service {
	t.Helper()
	svc, err := backend.New(
		n, md, signer, engine.LockSettlementPrice,
		backend.WithMarketDataRegistry(firstPartyMarketDataRegistry(t)),
		backend.WithMCPCatalogProvider(officerToolRegistry()),
	)
	if err != nil {
