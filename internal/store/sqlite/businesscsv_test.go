...
func TestApplyBusinessCSVImport_CreatesAccount(t *testing.T) {
		...
		Accounts: []BusinessCSVImportAccount{
			{
				Account: domain.Account{
					...
					Pnl: "4.5", PnlHaltReason: domain.PnlHaltReasonMissingInitialPnl, Notes: "note",
				},
				Exists: false,
			},
		},
	...
	if a.Code != "acc-1" || a.GroupCode != "grp" || a.Currency != "JPY" ||
		a.Pnl != "4.5" || a.PnlHaltReason != domain.PnlHaltReasonMissingInitialPnl || a.Notes != "note" {
...
func TestApplyBusinessCSVImport_UpdatesExistingAccount(t *testing.T) {
	...
	if err := rs.SetAccountPnl(
		ctx, "acc-2", "8.5", domain.PnlHaltReasonMissingFx,
	); err != nil {
	...
	if a.Currency != "CHF" || a.Notes != "updated" || !a.Blocked ||
		a.BlockReason != "kyc" || a.Pnl != "8.5" ||
		a.PnlHaltReason != domain.PnlHaltReasonMissingFx {
		t.Errorf("account = %+v", a)
	}
	// Engine id must be preserved on update.
...
func TestApplyBusinessCSVImport_ExplicitZeroResetsExistingAccountPnl(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	_, rs := newTestStore(t)
	if _, err := rs.CreateAccount(ctx, domain.Account{
		Code: "acc-pnl", Pnl: "8.5", PnlHaltReason: domain.PnlHaltReasonMissingFx,
	}); err != nil {
		t.Fatalf("CreateAccount: %v", err)
	}
	if err := rs.ApplyBusinessCSVImport(ctx, BusinessCSVImport{
		Accounts: []BusinessCSVImportAccount{{
			Account: domain.Account{Code: "acc-pnl", Pnl: "0"},
			Exists:  true, PnlSpecified: true,
		}},
	}); err != nil {
		t.Fatalf("ApplyBusinessCSVImport: %v", err)
	}
	account, ok, err := rs.GetAccount(ctx, "acc-pnl")
	if err != nil || !ok {
		t.Fatalf("GetAccount: ok=%v err=%v", ok, err)
	}
	if account.Pnl != "0" || account.PnlHaltReason != "" {
		t.Fatalf("account P&L = (%q, %q), want explicit zero without halt",
...
func TestApplyBusinessCSVImport_UpsertsBalance(t *testing.T) {
	...
	if b.Available != "100" || b.Held != "10" || b.RealizedPnl != "5" ||
		b.RealizedPnlHaltReason != domain.PnlHaltReasonArithmeticOverflow {
