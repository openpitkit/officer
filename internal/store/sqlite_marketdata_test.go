...
func TestMDInstanceDelete(t *testing.T) {
	...
	if err := rs.DeleteMarketDataInstance(ctx, created.ExternalID, true); err != nil {
	...
	if err := rs.DeleteMarketDataInstance(ctx, created.ExternalID, true); !errors.Is(err, domain.ErrNotFound) {
...
func TestMDCascadeDeleteInstance(t *testing.T) {
	...
	// Delete the instance.
	if err := rs.DeleteMarketDataInstance(ctx, inst.ExternalID, true); err != nil {
		t.Fatalf("DeleteMarketDataInstance: %v", err)
	}
