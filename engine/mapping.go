...
func balanceOutcomesFromList(
	...
	res idResolver,
) ([]fwengine.BalanceOutcome, error) {
	result := make([]fwengine.BalanceOutcome, 0, len(outcomes))
	for _, outcome := range outcomes {
		entry := outcome.Entry
		...
		result = append(result, fwengine.BalanceOutcome{Asset: asset, Outcome: accepted})
	}
	return result, nil
}

func mergeBalanceOutcomes(phases ...[]fwengine.BalanceOutcome) ([]fwengine.BalanceOutcome, error) {
	merged := make([]fwengine.BalanceOutcome, 0)
...
func executionReportPersistenceFrom(
...
) fwengine.ExecutionReportPersistence {
	...
	return fwengine.ExecutionReportPersistence{
		Trade:                trade,
		Commission:           in.Commission,
...
func executionBalanceSettlementsFrom(outcomes []fwengine.BalanceOutcome) []domain.BalanceSettlement {
	if len(outcomes) == 0 {
		return nil
	}
...
func immediateFillQuantity(
	o domain.Order, outcomes []fwengine.BalanceOutcome,
) (string, error) {
	switch o.AmountKind {
	case domain.OrderAmountKindQuantity:
