...
func TestLocalNode_InheritedCurrencyChangesBlockHaltedPnl(t *testing.T) {
	...
	t.Run("group currency", func(t *testing.T) {
		...
		n, st := newTestNode(t, eng)
		ctx := context.Background()
		createPublishedCurrencyAssets(t, n, "EUR", "GBP")
		if _, err := n.CreateGroup(ctx, domain.AccountGroup{
...
func TestLocalNode_GroupCurrencyIgnoresOwnCurrencyActiveOrder(t *testing.T) {
	...
	n, st := newTestNode(t, newFakeEngine())
	ctx := context.Background()
	createPublishedCurrencyAssets(t, n, "EUR", "JPY", "USD")
...
func TestLocalNode_SetAccountCurrencyBlockerReadFailureLeavesStateUntouched(t *testing.T) {
	...
	if getErr != nil || !ok || account.Currency != "EUR" {
		t.Fatalf("currency after preflight failure = store %+v ok=%v err=%v",
			account, ok, getErr)
	}
	assertFakeEffectiveCurrency(t, eng, id, "EUR")
}
...
type invalidCurrencyAssetEngineIDRealm struct {
	store.RealmStore
	asset string
}

type mismatchedCurrencyAssetEngineIDRealm struct {
	store.RealmStore
	asset string
}

func (r *invalidCurrencyAssetEngineIDRealm) GetAsset(
	ctx context.Context,
	code string,
) (domain.Asset, bool, error) {
	asset, ok, err := r.RealmStore.GetAsset(ctx, code)
	if code == r.asset && ok && err == nil {
		asset.EngineAssetID = 0
	}
	return asset, ok, err
}

func (r *mismatchedCurrencyAssetEngineIDRealm) GetAsset(
	ctx context.Context,
	code string,
) (domain.Asset, bool, error) {
	asset, ok, err := r.RealmStore.GetAsset(ctx, code)
	if code == r.asset && ok && err == nil {
		asset.EngineAssetID = domain.EngineAssetID(424242)
	}
	return asset, ok, err
}

type failAccountCurrencyStoreRealm struct {
	store.RealmStore
	err error
}

func (r *failAccountCurrencyStoreRealm) SetAccountCurrency(
	context.Context,
	domain.AccountID,
	string,
) error {
...
func TestLocalNode_SetAccountCurrencyValidatesStoredAssetBeforeEngine(t *testing.T) {
	t.Parallel()
	n, st := newTestNode(t, newFakeEngine())
	ctx := context.Background()
	createCurrencyAssets(t, st, "USD")
	...
	n.realm = &invalidCurrencyAssetEngineIDRealm{
		RealmStore: n.realm,
		asset:      "USD",
	}
	err := n.SetAccountCurrency(ctx, testKey(id), "USD", testCaller)
	...
	account, ok, getErr := st.GetAccount(ctx, id)
	if getErr != nil || !ok || account.Currency != "" {
		t.Fatalf("stored account = %+v ok=%v err=%v, want unchanged", account, ok, getErr)
	}
}

func TestLocalNode_SetAccountCurrencyRejectsStoredLiveAssetMismatch(t *testing.T) {
	t.Parallel()
	eng := newFakeEngine()
	eng.enforceResolver = true
	n, st := newTestNode(t, eng)
	...
	n.realm = &mismatchedCurrencyAssetEngineIDRealm{
		RealmStore: n.realm,
		asset:      "USD",
	}
	err := n.SetAccountCurrency(ctx, testKey(id), "USD", testCaller)
	...
	if strings.Contains(err.Error(), "424242") {
		t.Fatalf("SetAccountCurrency error discloses stored engine handle: %v", err)
	}
	account, ok, getErr := st.GetAccount(ctx, id)
	if getErr != nil || !ok || account.Currency != "" {
		t.Fatalf("stored account = %+v ok=%v err=%v, want unchanged", account, ok, getErr)
...
func TestLocalNode_SetAccountCurrencyPostEngineStoreFailureFatals(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	n, st := newTestNode(t, newFakeEngine())
	createCurrencyAssets(t, st, "EUR", "USD")
	...
	n.realm = &failAccountCurrencyStoreRealm{RealmStore: n.realm, err: cause}
	var fatalErr error
	n.fatal = func(err error) { fatalErr = err }
	...
	account, ok, getErr := st.GetAccount(ctx, id)
	if getErr != nil || !ok || account.Currency != "EUR" {
		t.Fatalf("stored account = %+v ok=%v err=%v, want EUR", account, ok, getErr)
	}
}
...
func TestLocalNode_GroupCurrencyGuardsOnlyEffectiveCurrencyChanges(t *testing.T) {
	...
	n, st := newTestNode(t, newFakeEngine())
	ctx := context.Background()
	createPublishedCurrencyAssets(t, n, "USD", "EUR", "JPY")
...
func TestLocalNode_DeleteGroupDetachesMembersWithCurrencyGuard(t *testing.T) {
	...
	t.Run("allowed without open balances", func(t *testing.T) {
		...
		assertFakeEffectiveCurrency(t, eng, "clear", "EUR")
...
func TestLocalNode_DefaultGroupCurrencyGuardsOnlyDefaultResolvedAccounts(t *testing.T) {
	...
	n, st := newTestNode(t, newFakeEngine())
	ctx := context.Background()
	createPublishedCurrencyAssets(t, n, "USD", "EUR", "JPY", "GBP")
...
func createPublishedCurrencyAssets(
	t *testing.T, n *localNode, codes ...string,
) {
	t.Helper()
	for _, code := range codes {
		if _, err := n.CreateAsset(
			context.Background(), domain.Asset{Code: code}, testCaller,
