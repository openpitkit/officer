...
func TestUpdateAccountRenamesIdentityWithDependentsAndImmutableAudit(t *testing.T) {
	...
	st, err := sqlite.New(t.TempDir()+"/account-rename.db", domain.DefaultRealm)
	if err != nil {
	...
	n, _, err := node.NewLocalNode(ctx, domain.DefaultRealm, st, func(snapshot engine.Snapshot) (engine.Engine, error) {
