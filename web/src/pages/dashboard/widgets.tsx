...
export function CountsRow({ counts }: DashboardReadyWidgetProps) {
  ...
  const tiles: {
  ...
  }[] = [
    {
      ...
      value: (
        <SlashCount left={counts.accountsActive} right={counts.accounts} />
      ),
    ...
    },
    {
      ...
      value: (
        <SlashCount
          values={[counts.ordersToday, counts.ordersActive, counts.ordersTotal]}
        />
      ),
                    ...
                    if (inst.state === "error") {
                      ...
                      return (
                        <Badge key={inst.id} variant="danger">
                          {t("marketData.instanceError")} {label}
                          {errSuffix || diagSuffix}
                        </Badge>
                      );
                    }
...
function AuditRow({ entry }: { entry: AuditEntry }) {
  ...
  return (
    <button
      type="button"
      onClick={() =>
        navigate(`/audit?source=${encodeURIComponent(entry.source)}`)
      }
